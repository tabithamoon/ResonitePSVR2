    ...
    public class IpcClient {
        ...
        public bool Start() {
            ...
            try {
                ...
                ResonitePSVR2.Error($"[IPC_CLIENT] Connection failed. LastError = {ex.SocketErrorCode}");
                return false;
            }
        }
            ...
            try {
                ...
                while ( m_running && !token.IsCancellationRequested ) {
                    ...
                    if ( readable && clientSocket.Available > 0 ) {
                        ...
                        if ( bytesRead <= 0 ) {
                            ResonitePSVR2.Warn("[IPC_CLIENT] Disconnected from server.");
                            break;
                        }

                        if ( bytesRead < Marshal.SizeOf<CommandHeader>() ) {
                            ResonitePSVR2.Warn("[IPC_CLIENT] Received invalid command header size.");
                            continue;
                        }
                ...
                if ( m_running ) {
                    ResonitePSVR2.Warn($"[IPC_CLIENT] Error in receive loop: {ex.Message}");
                }
            }
        }
                        ...
                        if ( header.dataLen == Marshal.SizeOf<CommandDataServerHandshakeResult>() ) {
                            ...
                            switch ( response.result ) {
                                ...
                                case EHandshakeResult.Failed: {
                                        ResonitePSVR2.Error("[IPC_CLIENT] Handshake failed!");
                                        break;
                                    }
                                case EHandshakeResult.Outdated: {
                                        ResonitePSVR2.Error($"[IPC_CLIENT] Handshake failed with reason: Outdated client. Please upgrade to an IPC version of {response.ipcVersion}");
                                        break;
                                    }
                            }
                        }
